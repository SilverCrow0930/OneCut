...
const jobQueue = new Map<string, SmartCutJob>()
...
// Dedicated AI function for Smart Cut with improved narrative-focused prompt
async function generateSmartCut(signedUrl: string, mimeType: string, job: SmartCutJob): Promise<any[]> {
...
Remember: For ${formatConfig.name}, the goal is to create ${job.videoFormat === 'long' ? 'a single cohesive video' : 'standalone clips'} that capture the most engaging and meaningful content from the source material.`
    ...
    try {
        ...
        console.log(`[SmartCut] Raw AI response:`, responseText.substring(0, 500))
        ...
        try {
            ...
            console.log(`[SmartCut] Direct parse failed, trying extraction strategies...`)
            ...
            if (jsonMatch) {
                try {
                    ...
                    console.log(`[SmartCut] Array extraction failed, trying object collection...`)
                }
            }
                ...
                if (objectMatches) {
                    clips = objectMatches.map(match => {
                        try {
                            ...
                            console.warn(`[SmartCut] Failed to parse object: ${match.substring(0, 100)}...`)
                            return null
                        }
                    }).filter(Boolean)
                }
            ...
            if (!clips.length) {
                console.log(`[SmartCut] Attempting manual field extraction...`)
        ...
        clips = clips.filter(clip => {
            ...
            if (duration < 5) {
                console.warn(`[SmartCut] Filtering out clip "${clip.title}" - too short (${duration}s)`)
                return false
            }
            return true
        })
        ...
        if (clips.length !== originalLength) {
            console.log(`[SmartCut] Filtered ${originalLength - clips.length} clips that were too short`)
        }
        ...
        if (clips.length < formatConfig.segmentCount.min || clips.length > formatConfig.segmentCount.max) {
            console.warn(`[SmartCut] Warning: ${clips.length} segments generated, expected ${formatConfig.segmentCount.min}-${formatConfig.segmentCount.max}. Continuing anyway.`)
        }
        ...
        if (job.videoFormat === 'short' && formatConfig.segmentLength) {
            for (let i = 0; i < clips.length; i++) {
                ...
                if (duration < formatConfig.segmentLength.min || duration > formatConfig.segmentLength.max) {
                    console.warn(`[SmartCut] Warning: Segment ${i+1} duration ${duration}s is outside expected range ${formatConfig.segmentLength.min}-${formatConfig.segmentLength.max}s. Continuing anyway.`)
                }
            }
        }
        ...
        if (job.videoFormat === 'short' && formatConfig.totalDuration) {
            ...
            if (totalDuration < targetWithTolerance.min || totalDuration > targetWithTolerance.max) {
                console.warn(`[SmartCut] Warning: Total duration ${totalDuration}s is outside target range ${targetWithTolerance.min}-${targetWithTolerance.max}s. Continuing anyway.`)
            }
        }

        console.log(`[SmartCut] Successfully extracted ${clips.length} ${job.videoFormat === 'long' ? 'segments for combination' : 'narrative segments'} with total duration ${totalDuration}s`)

        return clips
        
    } catch (error) {
        console.error('[SmartCut] AI generation failed:', error)
        throw error
    }
}

// Generate video description using AI
async function generateVideoDescription(signedUrl: string, mimeType: string, job: SmartCutJob, clips: any[]): Promise<string> {
    const { GoogleGenAI, createUserContent, createPartFromUri } = await import('@google/genai')
...
Return ONLY the description text, no extra formatting or quotes.`
    ...
    try {
        ...
        console.error('[SmartCut] Video description generation failed:', error)
...
// Add job to queue
export async function queueSmartCutJob(
    projectId: string,
    fileUri: string,
    mimeType: string,
    contentType: string,
    ...
    console.log(`[SmartCutProcessor] Job ${jobId} queued for project ${projectId}`)
...
async function updateProjectStatus(projectId: string, updates: any) {
    try {
        ...
        if (error) {
            console.error(`[SmartCutProcessor] Failed to update project ${projectId}:`, error)
        }
    } catch (error) {
        console.error(`[SmartCutProcessor] Database error updating project ${projectId}:`, error)
    }
}
...
async function processNextJob() {
    ...
    if (activeJobs.size >= MAX_CONCURRENT_JOBS) {
        console.log(`[SmartCutProcessor] At capacity (${activeJobs.size}/${MAX_CONCURRENT_JOBS})`)
        return
    }
    ...
    await processSmartCutJob(queuedJob)
...
async function processSmartCutJob(job: SmartCutJob) {
    try {
        console.log(`[SmartCutProcessor] Starting job ${job.id} for project ${job.projectId}`)
        
        // Get socket instance
        const io = global.io
        if (!io) {
            console.warn('[SmartCutProcessor] Socket.io instance not found')
        }
        
        // Helper function to emit state updates
        const emitState = (state: string, message: string, progress: number) => {
            if (io) {
                io.emit('smartcut_state', {
                    state,
                    message,
                    progress
                })
        ...
        const clips = await generateSmartCut(signedUrl, job.mimeType, job)
        ...
        job.progress = 100
        job.message = 'Smart Cut completed successfully!'
        ...
        console.log(`[SmartCutProcessor] Job ${job.id} completed successfully with ${processedClips.length} narrative segments`)
        
    } catch (error) {
        console.error(`[SmartCutProcessor] Job ${job.id} failed:`, error)
        ...
        // Emit error state
        if (global.io) {
            global.io.emit('smartcut_state', {
                state: 'error',
                message: job.message,
                progress: 0
            })
            global.io.emit('smartcut_response', {
                success: false,
                error: job.error
            })
        }
...
async function extractVideoClips(videoUrl: string, clips: AIGeneratedClip[], job: SmartCutJob): Promise<ProcessedClip[]> {
    ...
    try {
        ...
        console.error('[SmartCutProcessor] Source URL validation failed:', error)
    ...
    if (job.videoFormat === 'long') {
        try {
            ...
            console.error('[SmartCutProcessor] Failed to combine segments:', error)
            throw error
        }
    } else {
            ...
            try {
                ...
                await new Promise<void>((resolve, reject) => {
                    ffmpeg(videoUrl)
                        ...
                        .on('start', (command) => {
                            console.log(`[SmartCutProcessor] FFmpeg command for clip ${i}:`, command)
                        })
                        .on('progress', (progress: FFmpegProgress) => {
                            const percent = progress.percent ?? 0
                            console.log(`[SmartCutProcessor] Processing clip ${i}: ${Math.round(percent)}%`)
                        })
                        .on('end', () => resolve())
                        .on('error', (err) => reject(err))
                        .run()
                ...
                try {
                    ...
                    console.warn(`[SmartCutProcessor] Cleanup warning for job ${job.id}:`, cleanupError)
                }
                
            } catch (clipError) {
                console.error(`[SmartCutProcessor] Failed to process clip ${i} for job ${job.id}:`, clipError)
                throw clipError
            }
        }
    }
...
// Get job status
export function getJobStatus(jobId: string): SmartCutJob | null {
    return jobQueue.get(jobId) || null
}

// Get all jobs for a user
export function getUserJobs(userId: string): SmartCutJob[] {
...
cron.schedule('0 * * * *', () => {
    ...
    for (const [jobId, job] of jobQueue.entries()) {
        if (job.createdAt < cutoffTime && ['completed', 'failed'].includes(job.status)) {
            ...
            console.log(`[SmartCutProcessor] Cleaned up old job ${jobId}`)
        }
    }
})

console.log('[SmartCutProcessor] Background processor initialized') 